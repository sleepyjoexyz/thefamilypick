export interface StrollerArticle {
  slug: string;
  title: string;
  description: string;
  content: string;
...
Neither is objectively better; the choice depends on your lifestyle. Urban, travel-heavy, apartment-dwelling parents should lean lightweight. Suburban, car-based, park-loving parents should lean full-size. And if you have the budget and space, owning both eliminates the need to compromise.`,
  },
...
None of these choices are wrong. All offer proven features, reliable brands, and good value. The key is matching your specific needs—storage, reversibility, terrain, weight—to the model that solves your problem, rather than chasing the lowest price or the most expensive option.`,
  },
...
Never compromise on the three defining features: fixed front wheel, responsive suspension, and high-performance wheels. Everything else is preference.`,
  },
...
export function getStrollerArticle(slug: string): StrollerArticle | null {
  return strollerArticles[slug] || null;
}

export function getAllStrollerArticleSlugs(): string[] {
  return Object.keys(strollerArticles);
}
