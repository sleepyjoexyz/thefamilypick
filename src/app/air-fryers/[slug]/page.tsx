...
export default async function ArticlePage({ params }: PageProps) {
  ...
  if (!article) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <Link href="/air-fryers" className="text-blue-600 hover:underline mt-4 inline-block">
          Back to Air Fryers
        </Link>
      </div>
    );
  ...
  return (
    <div className="bg-white">
      ...
      {/* Comments Section */}
      <Comments pagePath={`/air-fryers/${slug}`} siteName="thefamilypick" />

      {/* Related Articles */}
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 border-t border-gray-200">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">Related Articles</h2>
        <Link href="/air-fryers" className="text-blue-600 hover:underline">
          ← Back to Air Fryers
        </Link>
      </section>
    </div>
