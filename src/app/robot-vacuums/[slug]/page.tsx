...
export async function generateStaticParams() {
  const slugs = getAllRobotVacuumArticleSlugs();
...
export default async function ArticlePage({ params }: PageProps) {
  ...
  return (
    <div className="bg-white">
      ...
      <CrossCategoryLinks currentCategory="robot-vacuums" />


      <Comments pagePath={`/robot-vacuums/${slug}`} siteName="thefamilypick" />

      {/* Related Articles */}
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 border-t border-gray-200">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">Related Guides</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {getAllRobotVacuumArticleSlugs()
            .filter((s) => s !== slug)
            .slice(0, 4)
            .map((relatedSlug) => {
              const related = getRobotVacuumArticle(relatedSlug);
              if (!related) return null;
              return (
                <Link
                  key={relatedSlug}
                  href={`/robot-vacuums/${relatedSlug}`}
