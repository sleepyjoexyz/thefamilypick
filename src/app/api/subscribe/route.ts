...
async function sendWelcomeEmail(email: string, siteName: string): Promise<boolean> {
  ...
  const siteConfig: { [key: string]: { title: string; description: string } } = {
    thefamilypick: {
      title: "The Family Pick",
      description: "Get the best family and lifestyle deals delivered to your inbox.",
    },
  };

  const config = siteConfig[siteName] || { title: "The Family Pick", description: "Get the best deals delivered to your inbox." };
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send welcome email
    const emailSent = await sendWelcomeEmail(trimmedEmail, "thefamilypick");
