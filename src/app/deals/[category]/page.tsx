...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!cat) return {};
  return {
    title: `${cat.name} Deals — Save Today | The Family Pick`,
    description: cat.description,
    alternates: { canonical: `https://www.thefamilypick.com/deals/${slug}` },
  };
}
...
export default async function CategoryDealsPage({ params }: Props) {
  ...
  return (
    <main className="bg-white min-h-screen">
      <DealSchema deals={categoryDeals} categoryName={cat.name} pageUrl={`https://www.thefamilypick.com/deals/${slug}`} />
      {/* Breadcrumbs */}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-6">
        ...
        <p className="text-sm text-gray-500">
          {categoryDeals.length} deals available — updated daily
        </p>
      </section>
      ...
      {/* Cross-links to other deal categories */}
